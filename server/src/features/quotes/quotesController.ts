...
const quotesFilePath = path.resolve(__dirname, '/home/younesnd/react-the-road-to-enterprise/server/src/features/quotes/quotes.json')
...
export const getTopQuotes = async (request: FastifyRequest, reply: FastifyReply) => {
  await sleep()
  const quotesData = await readQuotes()
  return {quotes: quotesData.quotes.slice(0, 5), }
}
...
const getQuotesByPage = async (page: number, limit: number) => {
  ...
  return {quotes, hasMore: endIndex < quotesData.quotes.length - 1}
}
...
export const getQuotes = async (request: FastifyRequest<GetQuotes>,reply: FastifyReply) => {
